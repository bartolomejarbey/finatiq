...
export default function CustomDomainPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(`/api/advisor-brand?domain=${encodeURIComponent(domain)}`);
        if (!res.ok) {
          setNotFound(true);
          setLoading(false);
          return;
  ...
  if (notFound || !advisor) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#060d1a] px-4">
        ...
        >
          Doména {domain} není přiřazena žádnému poradci.
        </p>
        <a
          href="https://www.finatiq.cz"
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#060d1a] px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md text-center">
        ...
        <div className="mt-10 space-y-4">
          <Link
            href={slug ? `/login?advisor=${encodeURIComponent(slug)}` : "/login"}
          ...
          >
            ...
            Přihlásit se jako poradce
          </Link>

          <Link
            href={slug ? `/portal/login?advisor=${encodeURIComponent(slug)}` : "/portal/login"}
